...
and the rest of the QMCPy team \\[2ex]
	...
	Slides at  \href{https://speakerdeck.com/fjhickernell/advances-challenges-qmc-software}{\nolinkurl{speakerdeck.com/fjhickernell/advances-challenges-qmc-software}}}
...
\begin{frame}{Quasi-Monte Carlo Cubature}
	...
	\[
	\uncover<2->{\mu :=  \int_{\ct} \alert<3>{g(\vt)} \, \alert<2>{\lambda(\vt) \, \dif \vt} \underbrace{=}_{\alert<2>{\vt = \vT(\vx)}} } \only<1>{\mu :=} 	\underbrace{\int_{[0,1]^d} \alert<3>{f(\vx)} \, \dif \vx}_{\text{integration}} =  \underbrace{\Ex[f(\vX)]}_{\text{expectation}} \approx  \underbrace{\frac 1{\alert<4>{n}} \sum_{i=1}^{\alert<4>{n}} f(\alert<1>{\vX_i})}_{\text{sample mean}} =: \hmu_{\alert<4>{n}}
	\]
...
\begin{frame}{Software Landscape}
	...
	% https://developer.nvidia.com/curand. CUDA
...
\begin{itemize}
	...
	\item \alert{Which} generators are significantly better for which practical problems?
\end{itemize}
}
...
\alert{Variable transformations} aka \alert{importance sampling}
	\begin{multicols}{2}
		{\Large \alert{Advances}}
		\begin{itemize}
		...
		\begin{itemize}
			...
			\item Need for adaptive importance sampling
...
\begin{frame}{Integrands or Use Cases}
	
	Choosing \alert{variable transformations}, $\vT: \ct \to [0,1]^d$ aka \alert{importance sampling}
	\begin{multicols}{2}
		{\Large \alert{Advances}}
		\begin{itemize}
		...
		\uncover<2>{
			{\Large \alert{Challenges}}
			\begin{itemize}
				\item Our use cases are refreshing
				
				\item Need to demonstrate connecting a package that computes the function values with the package that does the QMC
