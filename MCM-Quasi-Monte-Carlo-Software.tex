%Talk given virtually for MCM 2021
...
and the rest of the GAIL an QMCPy teams \\[2ex]
...
\begin{frame}{Quasi-Monte Carlo Cubature}
	...
	\[
	\uncover<2->{\mu :=  \int_{\ct} \alert<3>{g(\vt)} \, \alert<2>{\lambda(\vt) \, \dif \vt} \underbrace{=}_{\vt = \vPsi(\vx)}  } \only<1>{\mu :=} 	\underbrace{\Ex[f(\vX)]}_{\text{expectation}} = \underbrace{\int_{[0,1]^d} \alert<3>{f(\vx)} \, \dif \vx}_{\text{integration}} \approx  \underbrace{\frac 1{\alert<4>{n}} \sum_{i=1}^{\alert<4>{n}} f(\alert<1>{\vX_i})}_{\text{sample mean}} =: \hmu_{\alert<4>{n}}
	\]
	...
	\begin{description}[<+->]
		...
		\item[True Measure] that defines the original integral, e.g., Lebesgue or Gaussian, and performs the variable transformation
